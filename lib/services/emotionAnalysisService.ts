...
export class EmotionAnalysisService {
  ...
  private static readonly MAX_MESSAGES_FOR_ANALYSIS = 50;
  ...
  static prepareMessagesForAnalysis(messages: Message[]): Message[] {
    ...
    const recentMessages = meaningfulMessages.slice(
      -this.MAX_MESSAGES_FOR_ANALYSIS,
    );
  ...
  static generateConversationSuggestions(messages: Message[]): string[] {
    ...
    if (avgUserMessageLength < 30) {
      suggestions.push(
        "Try sharing more details about your thoughts and feelings",
      );
    }
    ...
    const hasEmotionWords = messages.some((msg) =>
      /\b(feel|felt|emotion|happy|sad|angry|anxious|excited|worried|proud|grateful)\b/i
        .test(msg.content)
    );

    if (!hasEmotionWords) {
      suggestions.push(
        "Describe how you're feeling to get more accurate emotional insights",
      );
    }
